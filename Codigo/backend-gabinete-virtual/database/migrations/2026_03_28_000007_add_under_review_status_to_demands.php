...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        if ($driver === 'pgsql') {
            DB::statement("ALTER TABLE demands DROP CONSTRAINT IF EXISTS demands_status_check");
            DB::statement(
                "ALTER TABLE demands ADD CONSTRAINT demands_status_check CHECK (status IN ('open', 'in_progress', 'completed'))"
            );
        ...
        if ($driver === 'mysql') {
            DB::statement(
                "ALTER TABLE demands MODIFY status ENUM('open', 'in_progress', 'completed') NOT NULL"
            );
        }
    }
};
